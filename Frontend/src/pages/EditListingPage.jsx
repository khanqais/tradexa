...
import { getListingById, updateListing, uploadImage } from '../api';
...
export default function EditListingPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        setForm({
          ...
          auction_ends_at: listing.auction_ends_at
            ? toLocalDateTimeString(new Date(listing.auction_ends_at))
            : '',
        });
        setImagePreviews(listing.image_urls || []);
      } catch (err) {
    ...
    if (isAuthenticated !== undefined) fetchListing();
  ...
  if (!isAuthenticated) {
    return (
      <div className="create-gate container">
        ...
        <p>You need an account to edit listings on Tradexa.</p>
  ...
  if (loading) {
    return (
      <div className="create-gate container" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Spinner />
      </div>
    );
  }
  ...
  if (notFound) {
    return (
      <div className="create-gate container" style={{ textAlign: 'center' }}>
        <h2>Listing not found</h2>
        <Link to="/my-listings" className="btn btn--ghost">← My Listings</Link>
      </div>
    );
  }
  ...
  if (forbidden) {
    return (
      <div className="create-gate container" style={{ textAlign: 'center' }}>
        ...
        <Link to={`/listings/${id}`} className="btn btn--ghost">← Back to Listing</Link>
      </div>
    );
  }
  ...
  return (
    <div className="create container">
      ...
      >
        <Link to={`/listings/${id}`} className="create__back">← Back to listing</Link>
      ...
      >
        <div className="create__layout">
          ...
          <div className="create__col-side">
            <div className="create__section">
              <h3 className="create__section-title">Item Photos</h3>
        ...
        <div className="create__actions">
          ...
          >
            {submitting || uploading ? (
              <><Spinner size="sm" /> {uploading ? 'Uploading image…' : 'Saving…'}</>
            ) : (
              'Save Changes'
            )}
          </button>
